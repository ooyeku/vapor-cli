        ...
        Self {
            ...
            columns: vec![
                ...
                ColumnConfig {
                    ...
                    distribution: DataDistribution::Normal { mean: 100.0, std_dev: 15.0 },
                    nullable: false,
                },
            ],
        }
...
pub fn populate_database(db_path: &str, config: Option<PopulationConfig>) -> Result<()> {
    ...
    println!("This may take a while. Progress will be shown every {} rows.", config.batch_size);
    ...
    match result {
        Ok(rows_inserted) => {
            ...
            println!("Successfully populated table '{}' with {} rows", config.table_name, rows_inserted);
...
fn estimate_row_size(columns: &[ColumnConfig]) -> usize {
    columns.iter().map(|col| match col.data_type {
        DataType::Integer => 8,
...
fn create_table_with_config(conn: &Connection, config: &PopulationConfig) -> Result<()> {
    let column_defs: Vec<String> = config.columns.iter().map(|col| {
    ...
    if existing_count > 0 {
        println!("Table '{}' already contains {} rows", config.table_name, existing_count);
...
fn populate_with_transaction(conn: &mut Connection, config: &PopulationConfig) -> Result<usize> {
    ...
    let placeholders = (0..config.columns.len()).map(|_| "?").collect::<Vec<_>>().join(", ");
            ...
            match stmt.execute(rusqlite::params_from_iter(values)) {
                Ok(_) => {
                    ...
                    if rows_inserted % config.batch_size == 0 {
                        ...
                        let eta = if rate > 0.0 {
                            Duration::from_secs(((config.row_count - rows_inserted) as f64 / rate) as u64)
                        } else {
                            Duration::from_secs(0)
                        };
                        
                        println!("Progress: {}/{} rows ({:.1}%) - {:.0} rows/sec - ETA: {:?}", 
                            rows_inserted, config.row_count, 
...
fn generate_row_values(columns: &[ColumnConfig], row_index: usize, rng: &mut StdRng) -> Vec<String> {
    columns.iter().map(|col| {
        if col.nullable && rng.gen_bool(0.1) { // 10% chance of NULL
            return "NULL".to_string();
        }
        
        match (&col.data_type, &col.distribution) {
            (DataType::Integer, DataDistribution::Sequential) => row_index.to_string(),
            (DataType::Integer, DataDistribution::Uniform) => rng.gen_range(0..1000).to_string(),
...
fn cleanup_failed_population(conn: &Connection, table_name: &str) -> Result<()> {
    ...
    if table_exists {
        ...
        if row_count > 0 {
            ...
            println!("Run 'DROP TABLE {};' to remove it, or try populating again", table_name);
        }
    }
...
fn verify_population_success(conn: &Connection, config: &PopulationConfig) -> Result<()> {
        ...
        |row| {
            ...
            for i in 0..config.columns.len() {
                ...
                let value = match config.columns[i].data_type {
                    ...
                    DataType::Date | DataType::Timestamp | DataType::UUID => row.get::<_, String>(i)?,
                };
                values.push(value);
            }
            Ok(values)
    ...
    println!("Final row count: {} rows in '{}'", final_count, config.table_name);
    
    if final_count >= config.row_count as i64 {
        println!("Population completed successfully!");
    } else {
        eprintln!("Warning: Expected at least {} rows but found {}", config.row_count, final_count);
    }
